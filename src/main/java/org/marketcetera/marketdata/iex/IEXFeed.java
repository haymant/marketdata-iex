...
/**
 * Market data feed implementation for the IEX market data supplier.
 *
...
@ClassVersion("$Id$")
class IEXFeed
        extends AbstractMarketDataFeed<IEXFeedToken,
                                       IEXFeedCredentials,
                                       IEXFeedMessageTranslator,
                                       IEXFeedEventTranslator,
                                       List<IEXRequest>,
                                       IEXFeed>
        implements IEXFeedServices
{
    /* (non-Javadoc)
    ...
    /* (non-Javadoc)
     * @see org.marketcetera.marketdata.iex.FeedServices#doDataReceived(java.lang.String, java.lang.Object)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see org.marketcetera.marketdata.iex.FeedServices#getRefreshInterval()
     */
    @Override
    ...
    protected void doCancel(String inHandle)
    {
        synchronized(requests) {
            IEXRequest request = requests.remove(inHandle);
            if(request != null) {
     ...
     */
    @Override
    protected boolean doLogin(IEXFeedCredentials inCredentials)
    {
     ...
     */
    @Override
    protected List<String> doMarketDataRequest(List<IEXRequest> inRequests)
            throws FeedException
    {
        List<String> handles = new ArrayList<String>();
        synchronized(requests) {
            for(IEXRequest request : inRequests) {
     ...
     */
    @Override
    protected IEXFeedToken generateToken(MarketDataFeedTokenSpec inTokenSpec)
            throws FeedException
    {
        return new IEXFeedToken(inTokenSpec,
                                  this);
    }
    /* (non-Javadoc)
     ...
     */
    @Override
    protected IEXFeedEventTranslator getEventTranslator()
    {
        return IEXFeedEventTranslator.INSTANCE;
    }
    /* (non-Javadoc)
     * @see org.marketcetera.marketdata.AbstractMarketDataFeed#getMessageTranslator()
     */
    @Override
    protected IEXFeedMessageTranslator getMessageTranslator()
    {
        return IEXFeedMessageTranslator.INSTANCE;
    }
    /**
    ...
    {
        return String.format("iex-%s", //$NON-NLS-1$
     ...
     */
    @GuardedBy("requests")
    private final Map<String,IEXRequest> requests = new HashMap<String,IEXRequest>();
    /**
